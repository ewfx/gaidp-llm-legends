...
def parse_rules_with_openai(text):
    ...
    5. For aggregated amounts: Specify the field name, type (e.g., "aggregated_amount") and threshold.
    ...
    [
        ...
        {{"type": "aggregated_amount", "field": "Total_Amount", "threshold": 2000}},
        {{"type": "high_frequency", "field": "Transaction_Amount", "time_period": "last_one_year", "threshold": 10}}
    ]

    IMPORTANT: 
    - Only consider the data provided in the current text. Do not use any historic data.
    - Your response must be valid JSON. Do not include any additional text or explanations.
    """
...
# Step 3: Pre-aggregate transaction data based on Customer_ID
...
def validate_batch_with_openai(batch, rules, start_index):
    ...
    IMPORTANT: 
    - Only consider the data provided in the batch. Do not use any external or historic data.
    - Your response must be valid JSON. Do not include any additional text or explanations.
    """
...
if __name__ == "__main__":
    ...
    pdf_path = "regulatory_document_latest.pdf"
